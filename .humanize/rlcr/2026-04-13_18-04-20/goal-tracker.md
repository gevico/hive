...
### Plan Version: 3 (Updated: Round 1 Re-anchor)
...
|------|-----------|--------|-----|-------|-------|
| task3: Frontmatter + schema validation | AC-16,AC-17 | pending | [queued] | claude | Keep queued unless a verifier contract fix requires a minimal schema change in the execution path |
| task5: Task ID + spec parser | AC-4 | pending | [queued] | claude | `spec_content_hash` and broader spec typing are out of scope for the round-1 mainline |
...
| task10: Layer 1 commands | AC-7,AC-9 | in_progress | [blocking] | claude | `launch` now owns `assigned -> in_progress` and `isolate` no longer advances state; context/skill injection remains queued |
| task11: hive check (3 verifiers) | AC-10 | in_progress | [mainline] | claude | `check` now gates `exec` with deterministic 0/1/2/3 exit codes; remaining work is broader fixture coverage and any result-recording follow-up |
| task12: hive report | AC-11 | in_progress | [blocking] | claude | `report` now uses typed `result.md` parsing and appends state-change audit; broader audit coverage is still pending |
| task13: hive exec orchestration | AC-8 | in_progress | [mainline] | claude | `exec` now reuses claim/isolate/launch/report/check helpers, honors verifier failures, and routes retries through `failed -> pending/blocked` |
...
|-------|-----------------|-------------|-----------------|
| Layer 1 command contracts drift from the planned pipeline (`launch` preconditions, deterministic `check` behavior) | 1 | AC-8, AC-9, AC-10 | Align launch/check semantics with the repaired execution path before declaring the mainline closed |
